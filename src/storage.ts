...
export async function importAllData(json: Record<string, unknown>): Promise<void> {
  if (json.pigbaby_data) {
    await saveData(json.pigbaby_data as AppData);
  }
  if (json.custom_tags) {
    await saveCustomTags(json.custom_tags as string[]);
  }
  if (json.tag_colors) {
  ...
  if (json.tasks) {
    await saveTasks(json.tasks as TasksData);
  }
  if (json.checkin) {
    await saveCheckIn(json.checkin as CheckInData);
  }
  if (json.tips) {
    await saveTips(json.tips as TipsData);
  }
  if (json.projects) {
    await saveProjects(json.projects as ProjectsData);
  }
  if (json.date_notes) {
    await saveDateNotes(json.date_notes as DateNotesData);
  }
}
